...
@Service
public class LogUserService implements Logable<PublicUser> {
	...
	public boolean isPersonUsable(Person person) {
		...
		if (person.getRole() == null || (!person.getRole().equals("doctor") && !person.getRole().equals("user"))) {
			return false;
		}
		...
		// DateOfBirth'un geçerli olup olmadığını ve aradaki tarih aralığına uygun olup
